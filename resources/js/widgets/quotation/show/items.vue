            ...
            <tbody>
                <tr v-if="(localQuotation.items || {}).length" v-for="(item, i) in localQuotation.items" :key="i"  :style=" ( (i + 1) % 2 ) ? 'background-color:'+secondaryColor+';' : ''">
                    <td colspan="4" class="p-2">
                        ...
                        <el-input v-if="editMode" :placeholder="'e.g) Item '+ (i+1)" v-model="localQuotation.items[i].name" size="mini" class="p-1" :style="{ maxWidth:'100%' }"></el-input>
                        
                        <p v-if="!editMode" class="mr-5">
                            <span v-if="!editMode">{{ item.description }}</span>
                        </p>
                        <el-input v-if="editMode" placeholder="e.g) Item" v-model="localQuotation.items[i].description" size="mini" class="p-1" :style="{ maxWidth:'100%' }"></el-input>
        ...
        methods: {
            ...
            addProductOrService(productsOrServices){
                ...
                for(var x = 0; x < productsOrServices.length; x++){
                    this.localQuotation.items.push(productsOrServices[x]);
                }
