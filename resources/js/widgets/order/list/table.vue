...
<script>
    ...
    export default {
        ...
        components: { 
            basicButton, Loader, orderPaymentStatusBadge, orderFulfillmentStatusBadge, orderRowDropDown, showActiveOrderWidget
        },
        data(){
            return {
                moment: moment,
        ...
        watch: {
            ...
            /** Watch for changes on the active order url. If we have an active url notify the
             *  parent by returing true, but if we dont have an active order url return false
             */ 
            activeOrderUrl: {
                handler: function (val, oldVal) {

                    //  If we have an active order url (The url of the order to view)
                    if( val ){

                        //  Notify the parent component that we aree viewing a specific order
                        this.$emit('viewingSingleRecord', true);

                    }else{

                        //  Notify the parent component that we are not viewing any specific order
                        this.$emit('viewingSingleRecord', false);
                ...
                if(this.tableColumnsToShowByDefault.includes('Payment Status')){
                    allowedColumns.push(
                    {
                        ...
                        render: (h, params) => {
                            ...
                            return h(orderPaymentStatusBadge, {
                                props: {
                ...
                if(this.tableColumnsToShowByDefault.includes('Fulfillment Status')){
                    allowedColumns.push(
                    {
                        ...
                        render: (h, params) => {
                            ...
                            return h(orderFulfillmentStatusBadge, {
                                props: {
