<template>
    ...
    <Row :gutter="20">
        ...
        <Col v-if="!company" span="20" offset="2">
            <!-- Loader -->
            <Loader v-if="true" :loading="true" type="text" class="text-left" theme="white">Loading invoices...</Loader>
        </Col>
        ...
        <Col v-else>
            ...
            <basicButton @click.native="$router.push({ name:'create-invoice', query: { clientId: company.id } })" size="large" class="float-right mb-2">
                + Create Invoice
            </basicButton>
            ...
            <activityCardWidget 
                :url="'/invoices/stats?companyId='+company.id" 
                :routePath="'/companies/'+company.id+'/invoices'"
                :isMoneyList="['Converted', 'Unconverted', 'Expired', 'Sent', 'Approved', 'Draft']">
            </activityCardWidget>

            <!-- Get the filterable Invoice list -->
            <invoiceListWidget :companyId="company.id"></invoiceListWidget>
...
<script>
    ...
    export default {
        components: { 
          Loader, pageToolbar, basicButton, overview, activityCardWidget, invoiceListWidget
        },
        data(){
            return {
                company: null,
