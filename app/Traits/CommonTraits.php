...
trait CommonTraits
{
     ...
     */
    public function scopeOnlyQueryWithinDateTime($query, $relation)
    {
        //  Only query for results where the created_at date is between the start date and end date
        $query->whereBetween('transactions'.'.created_at', [$this->getQueryStartDatetime(), $this->getQueryEndDatetime()])->get();
    }
