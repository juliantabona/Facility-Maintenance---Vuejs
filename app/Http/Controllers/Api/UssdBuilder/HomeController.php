...
class HomeController extends Controller
{
    ...
    public function handleRepeatScreenOnItems()
    {
        ...
        if (is_string($options) || is_integer($options) || is_float($options)) {

            //  Set an info log that we are converting the dynamic property to its associated value
            $this->logInfo('Converting <span class="text-success">'.$mustache_tag.'</span> to <span class="text-success">'.$options.'</span>');

        //  Incase the dynamic value is not a string, integer or float
        } else {

            //  Set an info log that we are converting the dynamic property to its associated value
            $this->logInfo('Converting <span class="text-success">'.$mustache_tag.'</span> to <span class="text-success">['.gettype($options).']</span>');
        ...
        if( is_array($options) ){
            ...
            for( $x = 0; $x < count($options); $x++ ){     
                ...
                if( !empty( $item_reference_name ) ){
                    ...
                    $this->storeDynamicData($item_reference_name, $options[$x]);
                ...
                if( !empty( $total_items_reference_name ) ){
                    ...
                    $this->storeDynamicData($total_items_reference_name, count($options));
                ...
                if( !empty( $is_last_item_reference_name ) ){
                    ...
                    $this->storeDynamicData($is_last_item_reference_name, (($x + 1) == count($options)));
                ...
                //  If we must navigate forward return null to stop and proceed to next iteration otherwise continue
                if ($buildResponse == 'navigate-forward') {

                    //  Do nothing so that we iterate tho the next item on the list
            ...
            $this->logWarning('The <span class="text-success">'.$mustache_tag.'</span> provided must be of type <span class="text-success">[Array]</span> however we received type of <span class="text-success">['.gettype($options).']</span>. For this reason we cannot repeat on options');
    ...
    public function handleCurrentDisplay()
    {
        ...
        if ($this->completedLevel($this->level)) {
            ...
            //  Check if we should navigate forward
        ...
        return $this->buildCurrentDisplay();
    ...
    public function buildCurrentDisplay()
    {
        ...
        $this->displayInstructions = $instructionsBuildResponse;
        ...
        //  Build the display actions (E.g Select options)
        $this->displayActions = $actionBuildResponse;

        //  Get the display instruction and action
        $result = $this->displayInstructions . $this->displayActions;

        //  If the processed instructions and action are not empty
        if (!empty($result)) {

            //  Set an info log of the final result
            $this->logInfo('Final result: <br /><span class="text-success">'.$result.'</span>');

        }

        //  Return the display instruction and action
        return $this->displayCustomPage($result);
    }
    ...
    public function getStaticSelectOptions($returnType = 'array')
    {
        ...
        if ($optionsExist) {
            ...
            for ( $x = 0; $x < count($options); ++$x ) {
                ...
                $this->logInfo('Option value: <span class="text-success">'.json_encode($option_value).'</span>');
        ...
        //  If we don't have options to display
            ...
            $text = (!empty($this->displayInstructions) ? "\n\n" : '');
            ...
            if (is_array($options)) {
                ...
                if ($optionsExist) {
                    ...
                    for ($x = 0; $x < count($options); ++$x) {
                        ...
                        }else if( !is_string( $option['name'] || is_integer( $option['name'] ) ) ){
    ...
    public function storeSelectedOption($options = [], $reference_name = null, $no_results_message = null, $incorrect_option_selected_message = null)
    {
        ...
        $selectedOption = collect(array_filter($options, function ($option) {
             ...
             */
            return urldecode( $this->currentUserResponse ) == $option['input'];
    ...
    public function canNavigateForward()
    {
        ...
        if ($this->screen['type']['selected_type'] == 'repeat') {
            ...
            if( $forward_navigation['selected_type'] == 'custom' ){
                ...
                $inputs = $forward_navigation['custom']['inputs'];
                ...
                if( !empty( $inputs ) ){
                    ...
                    if( in_array($this->currentUserResponse, $valid_inputs) ){
                        ...
                        //  Return true to allow forward navigation
                        return true;
            ...
            //  Return false to prevent forward navigation
            return false;
        }
    }
    ...
    public function handleEvent($event = null)
    {
        ...
        if( $event['active'] ){
            ...
            }else if( $event['type'] == 'SMS API' ){

                return $this->handle_SMS_API_Event();

            }else if( $event['type'] == 'Email API' ){

                return $this->handle_Email_API_Event();

            }else if( $event['type'] == 'Location API' ){

                return $this->handle_Location_API_Event();
                
            }else if( $event['type'] == 'Billing API' ){

                return $this->handle_Billing_API_Event();
                
            }else if( $event['type'] == 'Subcription API' ){
            ...
            }else if( $event['type'] == 'Custom Code' ){
    ...
    public function handle_CRUD_Api_Automatic_Response($response = null)
    {
        ...
        if ( $this->checkIfGoodStatusCode( $status_code ) ) {
            ...
            }else if( $on_success_handle_type == 'do_nothing' ){
        ...
        //  If this is a bad status code e.g "400", "401", "500" e.t.c
            ...
            }else if( $on_error_handle_type == 'do_nothing' ){
        ...
        try {
            ...
            if (!empty($response_status_handles)) {
                ...
                if ($selectedHandle) {
                    ...
                    foreach ($response_attributes as $response_attribute) {
                        ...
                        if (!empty( $name ) && !empty($value)) {
                            ...
                            if (is_string($output) || is_integer($output) || is_float($output)) {
                                ...
                                //  Set an info log that we are converting the dynamic property to its associated value
                                $this->logInfo(
                                    'Converting attribute: <span class="text-success">'.$mustache_tag.'</span> to <span class="text-success">'.$output.'</span> '.
                            ...
                            //  Incase the dynamic value is not a string, integer or float
                                ...
                                //  Set an info log that we are converting the dynamic property to its associated value
                                $this->logInfo(
                                    'Converting attribute: <span class="text-success">'.$mustache_tag.'</span> to <span class="text-success">'.json_encode($output).'</span> '.
                                    'and assigning the value to <span class="text-success">'.$response_attribute['name'].'</span>'
                                );
                    ...
                    }else if( $on_handle_type == 'do_nothing' ){
    ...
    public function formatCurrentScreenUserResponse()
    {
        ...
        if ($this->shouldDisplayScreen($failedFormatResponse)) return $failedFormatResponse;
    ...
    public function processPHPCode( $code = "return null", $log_dynamic_data = true )
    {
        ...
        try {
            ...
            if( count( $this->dynamic_data_storage ) ){
                ...
                if($log_dynamic_data) $this->logInfo('Creating variables using stored dynamic data');
                ...
                foreach ($this->dynamic_data_storage as $key => $value) {
                    ...
                    */
                    ...
                    if($log_dynamic_data) $this->logInfo('Variable <span class="text-success">$'.$key.'</span> = '.json_encode($value));
    ...
    public function storeDynamicData($name = null, $value = null, $log_status = true )
    {
        if( isset( $name ) && !empty( $name ) ){
            ...
            if( isset( $this->dynamic_data_storage[$name] ) ){
                ...
                if($log_status) $this->logWarning('Found existing data already stored within the reference name <span class="text-success">'.$name.'</span>, overiding the information.');
                
                //  Set an info log of the old data stored
                if($log_status) $this->logInfo('Old Data: <span class="text-success">'.json_encode($this->dynamic_data_storage[$name]).'</span>');

                //  Add the value as additional dynamic data to our dynamic data storage
                if($log_status) $this->dynamic_data_storage[$name] = $value;

                //  Set an info log of the new data stored
                if($log_status) $this->logInfo('New Data: <span class="text-success">'.json_encode($this->dynamic_data_storage[$name]).'</span>');
